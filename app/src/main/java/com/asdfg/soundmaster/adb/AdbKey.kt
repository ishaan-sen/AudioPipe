...
/**
 * ADB Key management - handles RSA key generation and storage
 * Extracted and adapted from Shizuku
 */
class AdbKey(private val adbKeyStore: AdbKeyStore, name: String) {
    ...
    private fun getOrCreatePrivateKey(): RSAPrivateKey {
        ...
        if (ciphertext != null) {
            try {
                ...
                Log.w(TAG, "Failed to decrypt stored key, generating new one", e)
            }
        }
        if (privateKey == null) {
    ...
    fun importPrivateKey(keyBytes: ByteArray) {
        try {
            val keyString = String(keyBytes)
            var newPrivateKey: RSAPrivateKey? = null

            // Try standard PKCS8 (PEM)
            if (keyString.contains("-----BEGIN PRIVATE KEY-----")) {
                 val base64 = keyString
                    .replace("-----BEGIN PRIVATE KEY-----", "")
                    .replace("-----END PRIVATE KEY-----", "")
                    .replace("\\s".toRegex(), "")
                val pkcs8Bytes = Base64.decode(base64, Base64.DEFAULT)
                val keyFactory = KeyFactory.getInstance("RSA")
                val privateKeySpec = PKCS8EncodedKeySpec(pkcs8Bytes)
                newPrivateKey = keyFactory.generatePrivate(privateKeySpec) as RSAPrivateKey
            } 
            // Try BouncyCastle PEMParser for other formats (like PKCS1)
            else if (keyString.contains("-----BEGIN")) {
                try {
                    val reader = java.io.InputStreamReader(java.io.ByteArrayInputStream(keyBytes))
                    ...
                    val keyPair = when (obj) {
                         is org.bouncycastle.openssl.PEMKeyPair -> converter.getKeyPair(obj)
                         is org.bouncycastle.asn1.pkcs.PrivateKeyInfo -> {
                             val privateKey = converter.getPrivateKey(obj)
                             if (privateKey is RSAPrivateKey) {
                                 // Return just the private key, no keypair wrapper effectively
                                 KeyPair(null, privateKey) 
                             } else null
                         }
                         else -> null
                    }
                    if (keyPair?.private is RSAPrivateKey) {
                        newPrivateKey = keyPair.private as RSAPrivateKey
                    }
                } catch (e: Exception) {
                    Log.w(TAG, "BouncyCastle PEM parse failed", e)
                }
            }

            // Fallback: Assume raw PKCS8 DER bytes if no PEM header
            ...
            if (newPrivateKey == null) {
                throw IllegalArgumentException("Could not parse private key. Supported formats: PKCS8 PEM, PKCS1 PEM, PKCS8 DER.")
            }
            ...
            val ciphertext = encrypt(newPrivateKey.encoded, aad)
            if (ciphertext != null) {
                adbKeyStore.put(ciphertext)
            }
        } catch (e: Exception) {
            Log.e(TAG, "Failed to import private key", e)
            throw RuntimeException("Invalid key format: ${e.message}")
        }
    }
...
// ADB Public Key encoding constants and functions
